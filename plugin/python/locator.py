...
class footPrintDrawOverride(omr.MPxDrawOverride):
    ...
    def prepareForDraw(self, objPath, cameraPath, frameContext, oldData):
        ...
        for i in range(soleCount - 1):
            data.fLineList.append(
                om.MPoint(sole[i][0] * fMultiplier, sole[i][1] * fMultiplier, sole[i][2] * fMultiplier))
            data.fLineList.append(
                om.MPoint(sole[i + 1][0] * fMultiplier, sole[i + 1][1] * fMultiplier, sole[i + 1][2] * fMultiplier))

        for i in range(heelCount - 1):
            data.fLineList.append(
                om.MPoint(heel[i][0] * fMultiplier, heel[i][1] * fMultiplier, heel[i][2] * fMultiplier))
            data.fLineList.append(
                om.MPoint(heel[i + 1][0] * fMultiplier, heel[i + 1][1] * fMultiplier, heel[i + 1][2] * fMultiplier))

        data.fTriangleList.clear()

        for i in range(1, soleCount - 1):
            data.fTriangleList.append(
                om.MPoint(sole[0][0] * fMultiplier, sole[0][1] * fMultiplier, sole[0][2] * fMultiplier))
            data.fTriangleList.append(
                om.MPoint(sole[i][0] * fMultiplier, sole[i][1] * fMultiplier, sole[i][2] * fMultiplier))
            data.fTriangleList.append(
                om.MPoint(sole[i + 1][0] * fMultiplier, sole[i + 1][1] * fMultiplier, sole[i + 1][2] * fMultiplier))
        ...
        for i in range(1, heelCount - 1):
            data.fTriangleList.append(
                om.MPoint(heel[0][0] * fMultiplier, heel[0][1] * fMultiplier, heel[0][2] * fMultiplier))
            data.fTriangleList.append(
                om.MPoint(heel[i][0] * fMultiplier, heel[i][1] * fMultiplier, heel[i][2] * fMultiplier))
            data.fTriangleList.append(
                om.MPoint(heel[i + 1][0] * fMultiplier, heel[i + 1][1] * fMultiplier, heel[i + 1][2] * fMultiplier))
...
def initializePlugin(obj):
    ...
    try:
        plugin.registerNode("footPrint", footPrint.id, footPrint.creator, footPrint.initialize, om.MPxNode.kLocatorNode,
    ...
    try:
        omr.MDrawRegistry.registerDrawOverrideCreator(footPrint.drawDbClassification, footPrint.drawRegistrantId,
